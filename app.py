...
# Mock de Banco de Dados de Força Técnica
TABELA_FORCA = {
...
# TTL de 300s (5 minutos) protege sua aplicação de ser banida pela API por excesso de chamadas
...
def buscar_jogos_do_dia(data_str: str, liga_slug: str = "all") -> list:
    ...
    try:
        ...
        resposta.raise_for_status() # Dispara erro se o HTTP não for 200 OK
        dados = resposta.json()
        ...
        for evento in dados.get('events', []):
            try:
                ...
                if data_utc_str:
                    try:
                        # Limpa frações de segundo se houver
...
def calcular_analise_real(f_casa: int, f_fora: int, linha_gols_api: float) -> dict:
    ...
    peso_casa = f_casa + 3 # Vantagem de mando de campo
    peso_fora = f_fora
    ...
    # Influência das Odds do Mercado nas probabilidades
